//
//  GVNNewScreeningOrderPackagesTableViewCell.swift
//  GlobalVerificationNetwork-HUCM
//
//  Created by Chetu India on 10/05/17.
//

import UIKit

class GVNNewScreeningOrderBronzePackageTableViewCell: GVNNewScreeningOrderBaseTableViewCell {
  
  
  // MARK:  UITableViewCell subclass GVNHomeTableViewCell widget instance veriable declaration.
  
  
  /*
   * Method awakeFromNib to set default property on UI widget elements.
   */
  // MARK:  awakeFromNib method.
  override func awakeFromNib() {
    super.awakeFromNib()
    // Initialization code
  }
  
  override func setSelected(_ selected: Bool, animated: Bool) {
    super.setSelected(selected, animated: animated)
    // Configure the view for the selected state
  }
  
}
